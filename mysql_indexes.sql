...
-- Task 1: Rewrite "SELECT * FROM Orders;" statement so that it is optimized. data they need is contained within the OrderID, ProductID, Quantity and Date columns. 
SELECT OrderID, ProductID, Quantity, Date 
FROM Orders;
-- Task 2: Lucky Shrub need to find the order placed by the client Cl1. They have written the following query to complete this task: SELECT * FROM Orders WHERE ClientID ='Cl1'; | Help Lucky Shrub to optimize this query by creating an index named IdxClientID on the required column of the Orders table. Once you have created the index, run the same SELECT statement as above with the EXPLAIN statement.
CREATE INDEX IdxClientID ON Orders(ClientID);

EXPLAIN SELECT * 
FROM Orders 
WHERE ClientID='Cl1';
-- Task 3: Lucky Shrub have written the following SELECT query to find the details of the employee whose last name is 'Tolo' SELECT * FROM Employees WHERE FullName LIKE '%Tolo'; rewrite the SELECT query and improve its performance so that it uses a trailing wildcard instead of the leading wild card.
